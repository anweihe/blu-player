      ...
      <div class="h-14 flex items-center gap-3 px-4 safe-area-left safe-area-right">
          ...
          @if (config().leftAction === 'back') {
            ...
            @if (config().showPlayButton) {
              <button
                ...
                (click)="playAll()"
                [attr.aria-label]="'qobuz.playAll' | translate"
              >
        ...
        <div class="flex-1 min-w-0">
          <span class="font-semibold text-lg text-text-primary truncate block">
            {{ config().title ? (config().title! | translate) : 'Bluesound' }}
          </span>
        </div>
            ...
            @switch (item) {
              @case ('search') {
                <button
                  class="w-10 h-10 flex items-center justify-center text-text-secondary hover:text-text-primary transition-colors"
                  (click)="openSearch()"
                  [attr.aria-label]="'navPreset.search' | translate"
                >
              ...
              @case ('profile') {
                <button
                  ...
                  (click)="toggleMenu.emit()"
                  [attr.aria-label]="'profile.defaultName' | translate"
                >
                  {{ profileInitial() }}
                </button>
              }
