...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, RouterLinkActive, ProfileSwitcherComponent, TranslatePipe],
  template: `
        ...
        <div class="border-t border-border-subtle p-4">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              @if (auth.isLoggedIn()) {
                <p class="font-medium truncate">{{ profileService.activeProfile()?.name ?? auth.user()?.display_name ?? ('profile.defaultName' | translate) }}</p>
                <p class="text-xs text-text-muted truncate">{{ auth.user()?.email }}</p>
              } @else {
                <p class="font-medium truncate">{{ profileService.activeProfile()?.name ?? ('profile.noProfile' | translate) }}</p>
                <p class="text-xs text-text-muted">{{ 'profile.notLoggedInShort' | translate }}</p>
              }
              <p class="text-xs text-accent-qobuz mt-0.5">{{ 'profile.switchProfile' | translate }}</p>
            </div>
          ...
          @if (auth.isLoggedIn()) {
            ...
            >
              ...
              {{ 'auth.logoutFromQobuz' | translate }}
            </button>
          } @else {
            <a
...
export class HamburgerMenuComponent implements OnInit {
  ...
  readonly menuItems: MenuItem[] = [
    ...
    {
      labelKey: 'nav.airable',
      route: '/airable',
      icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 22V12h6v10"/></svg>`
    },
    {
      labelKey: 'nav.radioParadise',
      route: '/radioparadise',
